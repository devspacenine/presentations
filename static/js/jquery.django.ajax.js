	...
	*/
	$.extend($ite.animations, {
		...
		fade: function($target, new_html) {
			...
			new_content = $('<div class="new-content-wrapper"/>').html(new_html).css({opacity: 0, width: start_width, height: start_height});
			$target.css({'position': 'relative', 'display': 'block', 'height': start_height, 'width': start_width}).wrapInner('<div class="old-content-wrapper"/>').children().css({'margin-bottom': '-'+start_height+'px','margin-left': '-'+start_width+'px'}).animate({'opacity': 0}, 500, function(){
                $target.removeAttr('style');
				$(this).remove();
			});
			$target.append(new_content).animate({'height': new_content.height(), 'width': new_content.width()}, 1000).children(':last').animate({'opacity': 1}, 1000, function(){
				$(this).children().unwrap();
				$target.removeAttr('style');
				dfd.resolve();
			});
